...
def celfpp_im(network, budget, diffusion_model, n_sim=100, all_upto_budget=True):
    ...
    celfpp_folder_new = celfpp_folder+'_copies'+os.sep+'celf++_'+network.name[1:] 
    ...
    "#### Getting the runtimes (cumulative in minutes)"
    run_times = [x.split(' ')[5] for x in open(out_filename).readlines()] 
    run_times = [float(x) for x in run_times]
    
    "##### Saving the all runtimes to a text file"
    run_times = {'celfpp':run_times}
    fstr = results_folder_runtime_files+os.sep+'runtime_info_celfpp_all.txt'
    with open(fstr, 'w') as f:
        f.write(json.dumps(run_times))
