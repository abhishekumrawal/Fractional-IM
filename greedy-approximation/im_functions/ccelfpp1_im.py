...
def ccelfpp1_im(network, budget, diffusion_model, n_sim=100, 
    ...
    logging.info('Number of communities detected/provided is '+str(len(communities))+'.')
    logging.info('Sizes of the communities are as follwows.')
    logging.info(str([len(community) for community in communities]))
    logging.info('Communities form a valid partition: ' + str(is_partition(network, communities))+'.')  
    logging.info('Coverage of the partition is ' + str(coverage(network, communities)))
    #logging.info('Performance of the partition: ' + str(performance(network, communities))+'.')
    logging.info('Modularity of the partition is ' + str(modularity(network, communities))+'.')
    ...
    for i,community in enumerate(communities):
        ...
        best_seed_set, exp_influence, runtime = celfpp_im(sub_network, budget_for_greedy, diffusion_model, n_sim, all_upto_budget=True)
    ...
    if all_upto_budget == True:
        ...
        results = {'budget':budget, 'diffusion_model':diffusion_model, 'algorithm':'ccelfpp1', 'n_sim':n_sim, \
                   'community_method':community_method, 'network_name':network.name, \
